...
import { ArrowLeft, Upload, Sparkles, Check, ChevronRight, AlertCircle, X, Camera } from 'lucide-react';
import { removeBackground } from '@imgly/background-removal';
import { CONDITIONS, CATEGORIES, type Condition, type Category } from '../lib/types';
...
// Compress image to at most maxBytes before sending to the edge function.
// Supabase edge functions have a ~6MB body limit.
async function compressForUpload(file: File, maxBytes = 4 * 1024 * 1024): Promise<File> {
  if (file.size <= maxBytes) return file;

  const bitmap = await createImageBitmap(file);
  let { width, height } = bitmap;

  // Scale down proportionally until we're likely under the limit
...
// Try remove.bg via Edge Function first (server-side, high quality).
// Falls back to @imgly/background-removal (client-side) if unavailable.
// Returns { blob, source } so the caller can show which engine ran.
async function removeBackgroundHQ(file: File): Promise<{ blob: Blob; source: 'removebg' | 'local' }> {
  try {
    const { data: { session } } = await supabase.auth.getSession();
    const compressed = await compressForUpload(file);

    console.log('[bg] calling remove-bg edge function, file size:', compressed.size);
    ...
    const res = await fetch(`${SUPABASE_URL}/functions/v1/remove-bg`, {
      ...
      body: (() => {
        ...
        form.append('image_file', compressed, compressed.name);
        return form;
      })(),
    });

    console.log('[bg] edge function status:', res.status);

    if (res.ok) {
      const blob = await res.blob();
      console.log('[bg] remove.bg blob size:', blob.size, 'type:', blob.type);
      if (blob.size > 1000 && blob.type.includes('image')) {
        console.log('[bg] ✅ using remove.bg');
        return { blob, source: 'removebg' };
      }
    } else {
      const errText = await res.text().catch(() => '');
      console.warn('[bg] ❌ edge function error', res.status, errText);
    }
  } catch (err) {
    console.warn('[bg] ❌ edge function unreachable', err);
  }

  console.log('[bg] ⚠️ falling back to local @imgly model');
...
export default function SellPage() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      const { blob } = await removeBackgroundHQ(file);

      const cleanUrl = URL.createObjectURL(blob);
      setImageCleanUrl(cleanUrl);
      ...
      // Background removal failed, proceed with original photo
    }
  ...
  async function handleStyledPhotoFile(index: number, file: File) {
    ...
    try {
      ...
      // proceed with original if background removal fails
    }

    const fileName = `${user.id}/styled_${Date.now()}_${index}.png`;
    const { error } = await supabase.storage.from('listing-images').upload(fileName, uploadBlob);
    if (!error) {
  ...
  async function handlePublish() {
    ...
    if (!sp) {
      console.error('No seller_profile found for user:', user.id);
      setSubmitting(false);
    ...
    if (error || !data) {
      console.error('Listings insert error:', JSON.stringify(error));
      setSubmitting(false);
      setPublishError(true);
      return;
    }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      {step === 1 && (
        <div className="bg-white rounded-[24px] border border-zinc-100 p-6 sm:p-8">
          ...
          <p className="text-sm text-zinc-400 mb-2">Clothing items only — tops, bottoms, shoes, bags, and accessories.</p>
          <p className="text-xs text-zinc-400 mb-6">We'll remove the background automatically. JPG, PNG or WebP · Max 10MB.</p>
          ...
          {!photo ? (
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-full max-w-sm aspect-square rounded-2xl overflow-hidden bg-white border border-zinc-100 mb-4">
                <img
                  src={imageCleanUrl || photo}
                  alt="Upload"
                  className={`w-full h-full object-cover transition-all duration-500 ${processing ? 'opacity-50' : 'opacity-100'}`}
                />
                {processing && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    <div className="relative flex items-center gap-2 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full">
                      <Sparkles size={14} className="text-zinc-400 animate-pulse-soft" />
                      <span className="text-xs font-medium text-zinc-400">Removing background...</span>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[0, 1, 2].map((index) => (
                    <div key={index} className="aspect-square relative">
                      ...
                      <input
                        ref={styledFileRefs[index]}
                        type="file"
                        accept="image/jpeg,image/png,image/webp"
                        className="hidden"
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) handleStyledPhotoFile(index, file);
                          e.target.value = '';
                        }}
                      />
                    </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CONDITIONS.map((c) => (
                    <button
                      key={c}
                      onClick={() => { setCondition(c); setErrors((prev) => ({ ...prev, condition: undefined })); }}
              ...
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-none">
                  {CATEGORIES.filter((c) => c !== 'All').map((c) => (
                    <button
                      key={c}
                      onClick={() => { setCategory(c); setErrors((prev) => ({ ...prev, category: undefined })); }}
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="w-10 h-10 rounded-full border border-zinc-200 bg-zinc-100 text-zinc-700 text-lg font-medium flex items-center justify-center hover:bg-zinc-200 transition-colors"
                  >−</button>
        ...
        <div className="bg-white rounded-[24px] border border-zinc-100 p-6 sm:p-8">
          ...
          <div className="flex gap-5 sm:gap-6">
            ...
            <div className="flex-1 min-w-0 space-y-2">
              ...
              {size && (
                <p className="text-sm text-zinc-600">Size: {size}</p>
              )}
              {brand && (
                <p className="text-sm text-zinc-600">Brand: {brand}</p>
              )}
              {description && (
